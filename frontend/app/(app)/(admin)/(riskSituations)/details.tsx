...
export default function RiskSituationDetailsScreen() {
  ...
  useEffect(() => {
    // Extraer riskSituation de los parámetros
    if (params.riskSituation) {
      const parsedData = JSON.parse(params.riskSituation as string);
      // Asegurarse que voluntariosTransporte sea un array
      setRiskSituation({
        ...parsedData,
        voluntariosTransporte: parsedData.voluntariosTransporte || []
      });
    }
  
    // Mantener el fetch de predefinedItems
  ...
  }, [params.riskSituation]);

  // El resto del código permanece igual, solo cambia el handleSave
  const handleSave = async () => {
    if (!riskSituation) return;
    try {
      // Aquí podrías agregar la lógica para actualizar en Firestore si lo necesitas
    ...
    return (
      <View style={styles.section}>
        ...
        <View style={styles.formGroup}>
          ...
          {riskSituation.voluntariosTransporte && riskSituation.voluntariosTransporte.length && riskSituation.voluntariosTransporte.map((route, index) => (
          ...
          {riskSituation.voluntariosTransporte && !riskSituation.voluntariosTransporte.length && <Text style={styles.input}>No hay rutas</Text>}
          {isEditing && (
            <TouchableOpacity
